...
export async function testGoogleMapsAPIs(apiKey: string): Promise<{
  mapsJsWorks: boolean;
  ...
  try {
    ...
    results.geocodingWorks =
      geocodingResponse.status === 200 && geocodingData.status === "OK";
  ...
  try {
    ...
    if (pollenResponse.status === 403) {
      results.error =
        "Pollen API returned 403 Forbidden. The API may not be enabled for your project.";
    } else if (pollenResponse.status === 404) {
      results.error =
        "Pollen API returned 404 Not Found. The API endpoint might have changed or is not available.";
    }
    ...
    if (!results.pollenApiWorks) {
      try {
        ...
        console.log("Pollen API response:", responseText);
      } catch (e) {
